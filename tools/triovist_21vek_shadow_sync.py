#!/usr/bin/env python3
"""Shadow synchronization for the Resanta/Triovist 21vek parser.
...
Approved scope excludes:
questions/answers, SALE/action fields, pickup Minsk and external HD/YouTube
...
def snapshot_row(run_id: str, source: dict, parsed: dict|None, error: str|None=None) -> dict:
    ...
    if parsed is None:
        return {
          ...
          "sku":source.get("sku"),
          "donor_article":source.get("donor_article"),
    ...
    return {
      ...
      "warranty_count":warranty.get("count"),
      "warranty_unit":warranty.get("unit"),
      "photo_count":card.get("photo_count"),
      "video_count":card.get("video_count"),
      "product_rating":card.get("product_rating"),
      "last_review_rating":card.get("last_review_rating"),
      "review_count":card.get("review_count"),
      "negative_reviews":card.get("negative_reviews"),
      "unanswered_negative_reviews":card.get("unanswered_negative_reviews"),
      "latest_review_date":card.get("latest_review_date"),
      "latest_review_text":card.get("latest_review_text"),
      "latest_review_answered":card.get("latest_review_answered"),
