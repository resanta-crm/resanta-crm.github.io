#!/usr/bin/env python3
"""
Импорт отчёта ПДЗ (задолженность клиентов) из почты в Supabase.
...
Как работает:
  ...
  4. Полностью перезаписывает таблицу client_debt в Supabase актуальными долгами.
  5. Помечает письмо прочитанным, чтобы не обрабатывать его повторно.

Структура отчёта 1С (важно для парсера):
  - Строка менеджера: имя в колонке 0, пусто в колонке 1, есть сумма в колонке 12.
  - Строка клиента:   номер в колонке 0, ИМЯ КЛИЕНТА в колонке 1, сумма в колонке 12.
  - Строка накладной: номер в колонке 0, пусто в колонке 1, документ в колонке 9.
  Нас интересуют только строки клиентов (итог по клиенту), детализация по
  накладным не нужна — так решили при проектировании.
