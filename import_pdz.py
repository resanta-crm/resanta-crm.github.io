...
LOOKBACK_DAYS = int(os.environ.get("PDZ_LOOKBACK_DAYS", "14"))
...
def parse_pdz(xlsx_bytes):
    """Разбирает Excel отчёта ПДЗ.

    Возвращает (строки просрочки, дата отчёта, метаданные контроля).
    Метаданные позволяют безопасно отличить действительно нулевую ПДЗ от
    сломанного/пустого отчёта: мы должны увидеть строки менеджеров и их итоги.
    """
    ...
    for _, row in df.iterrows():
        ...
        if pd.notna(col1):
            ...
            if name and name not in ("№ в группе", "Итого", "№ п/п"):
                ...
                if is_allowed_manager(name):
                    short = next((x for x in ALLOWED_MANAGERS if x.lower() in name.lower()), name)
                    recognized_managers.add(short)
                    manager_overdue_totals[short] = (num(row[c_overdue]) or 0) if c_overdue is not None else 0

    meta = {
        "recognized_managers": sorted(recognized_managers),
        "manager_overdue_totals": manager_overdue_totals,
        "validated_zero": bool(recognized_managers) and all((v or 0) <= 0 for v in manager_overdue_totals.values()),
    }
...
def main():
    ...
    for msg_id in ids:
        ...
        if not content:
            ...
            continue
        try:
            rows, report_date, meta = parse_pdz(content)
            parsed_candidates.append((report_date or date.min, sent, subject, filename, rows, report_date, meta))
        except Exception as exc:
    ...
    _, sent, subject, filename, rows, report_date, meta = parsed_candidates[-1]
    ...
    if not rows and not meta.get("validated_zero"):
        raise RuntimeError(
            ...
            "Прежний срез сохранён — проверьте структуру отчёта в 1С."
        )
    if not rows:
        log("  ✅ Отчёт подтверждает нулевую ПДЗ по менеджерам: " + ", ".join(meta.get("recognized_managers") or []))
    ...
    safe_set_import_status(
        "pdz", "ok", report_date=report_date.isoformat(),
        source_message_at=sent.isoformat(), row_count=len(rows),
        details=("ПДЗ отсутствует — отчёт проверен" if not rows else f"Просрочено {sum(row['debt_overdue'] for row in rows):.2f} BYN; файл {filename}"),
    )
    mail.logout()
    log("Готово.")
