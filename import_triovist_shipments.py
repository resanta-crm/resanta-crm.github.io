...
v22.7.29 — stable Gmail transport:
- Gmail endpoint is pinned to imap.gmail.com:993 even if an empty/invalid IMAP_HOST/PORT
  variable is present in GitHub Actions;
- 3 connection attempts for transient network/IMAP aborts;
- searches Gmail All Mail like the already proven sales importers;
- reads Subject/Date in batches, not one IMAP FETCH per message;
- no report / temporary Gmail problem never deletes existing shipment data.
...
MAX_FULL_MESSAGES = max(1, min(20, int(_env_text("TRIOVIST_SHIPMENTS_MAX_EMAIL_CANDIDATES", "8"))))
...
def attachment(msg):
    for p in msg.walk():
        ...
        if fn.lower().endswith(".xlsx"):
            return fn, p.get_payload(decode=True)
    return None, None
...
def fetch_message_headers_batch(mail, message_ids):
    ...
    for item in data or []:
        ...
        result.append({"id": message_id, "subject": decode(hdr.get("Subject")), "sent": msg_date(hdr.get("Date"))})
    return result


def recent_subject_candidates(mail, ids):
    """Batch-scan headers and return newest matching reports."""
    candidates = []
    ...
    for batch_no, start in enumerate(range(0, len(ids), HEADER_BATCH_SIZE), 1):
        ...
        for meta in metas:
            ...
            if SUBJECT_KEYS and not all(key in subject_n for key in SUBJECT_KEYS):
                continue
            candidates.append(meta)
    candidates.sort(key=lambda x: x["sent"], reverse=True)
    log(f"Найдено писем по отгрузкам Триовист: {len(candidates)}.")
...
def main():
    ...
    try:
        ...
        ids = data[0].split()
        log(
            f"Писем за последние {LOOKBACK} дн.: {len(ids)}. "
            f"Заголовки получаю пакетами по {HEADER_BATCH_SIZE}."
        )
        candidates = recent_subject_candidates(mail, ids)
        if not candidates:
            log("ℹ️ Свежего отчёта отгрузок Триовист не найдено. Это не ошибка; данные не менялись.")
            return
        ...
        for idx, meta in enumerate(candidates, 1):
            ...
            try:
                ...
                if not rows:
                    raise RuntimeError("в отчёте нет строк ООО Триовист после проверки артикула/900 группы")
                n = upsert(rows)
                log(f"✅ Отгрузки Триовист: {n} строк из {fn}, письмо {sent:%d.%m.%Y %H:%M}")
                return
            except Exception as e:
                errors.append(f"{sent:%d.%m %H:%M} · {fn}: {e}")
                log("  ⚠️ " + errors[-1])

        detail = "; ".join(errors[-5:]) if errors else "нет пригодных Excel-вложений"
        raise RuntimeError("Подходящие письма найдены, но ни один Excel не удалось разобрать. " + detail)
    finally:
        try:
            mail.logout()
        except Exception:
