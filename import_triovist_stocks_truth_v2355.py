...
from import_triovist_stocks import dec, extract_internal_sku, is_group900_sku, log, norm, parse_chekhov, upload_staged
...
def _find_header(ws) -> tuple[int, list[str]]:
    for row_idx, values in enumerate(ws.iter_rows(min_row=1, max_row=min(ws.max_row, 35), values_only=True), start=1):
        ...
        if any(v == "номенклатура" or v.startswith("номенклатура") or v.startswith("наименование") for v in keys) and any(v == "всего" or v.startswith("всего") for v in keys):
...
def parse_partner_truth(path: str | Path):
    ...
    if c_sales3 < 0 and len(legacy_sales) < 3:
        raise RuntimeError("Не найдены продажи 21vek: нужна колонка «Продажи за 3 мес.» либо три старые колонки «Продажи за ...». «Статистика продаж» намеренно не используется.")
    ...
    explicit_cols = []
    for name in ("Наш артикул", "Артикул поставщика", "Артикул производителя", "SKU поставщика", "SKU производителя", "Код поставщика", "Код производителя", "Наш SKU"):
        idx = _header_index(headers, name)
        if idx >= 0 and idx not in explicit_cols:
            explicit_cols.append(idx)
    rows = []
    diagnostics = {"version": VERSION, "header_row": header_row, "sales_mode": "three_month_total" if c_sales3 >= 0 else "legacy_three_months", "sales_column": headers[c_sales3] if c_sales3 >= 0 else " + ".join(headers[i] for i in legacy_sales), "stat1_column": headers[c_stat1] if c_stat1 >= 0 else None, "stat2_column": headers[c_stat2] if c_stat2 >= 0 else None, "forecast_column": headers[c_forecast] if c_forecast >= 0 else None, "free_now_column": headers[c_free_now] if c_free_now >= 0 else None, "free_transit_column": headers[c_free_transit] if c_free_transit >= 0 else None, "samples": []}
    for row_no, values in enumerate(ws.iter_rows(min_row=header_row + 1, values_only=True), start=header_row + 1):
        ...
        if not sku or is_group900_sku(sku):
            continue
        ...
        free_transit = max(0.0, _n(values[c_free_transit]) if c_free_transit >= 0 and c_free_transit < len(values) else 0.0)
        ...
        note = {"truth_version": VERSION, "sales_3m": sales_90, "stat_sales_2": _n(values[c_stat2]) if c_stat2 >= 0 and c_stat2 < len(values) else None, "stat_sales_1": _n(values[c_stat1]) if c_stat1 >= 0 and c_stat1 < len(values) else None, "total": raw_total, "free_now": free_now, "reserve": reserve, "in_transit": transit, "free_in_transit": free_transit, "partner_forecast": partner_forecast}
        rows.append({"row_key": f"{partner_sku or sku}|{sku}|{product}", "partner_sku": partner_sku or sku, "sku": sku, "kind": kind, "brand": str(values[c_brand] or "").strip() if c_brand >= 0 and c_brand < len(values) else "", "product": product, "sales_m1": "0", "sales_m2": "0", "sales_m3": str(sales_90), "sales_m1_label": "CRM truth: не используется", "sales_m2_label": "CRM truth: не используется", "sales_m3_label": headers[c_sales3] if c_sales3 >= 0 else "CRM truth: сумма 3 старых месяцев", "qty_total": str(available), "qty_free": str(available), "qty_transit_reserve": str(free_transit + reserve), "qty_orders": str(orders), "excluded": excluded, "match_note": json.dumps(note, ensure_ascii=False, separators=(",", ":"))})
