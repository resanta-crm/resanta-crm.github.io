...
def latest_attachment() -> tuple[bytes, str, datetime]:
    ...
    try:
        ...
        candidates: list[tuple[datetime, bytes, str]] = []
        for msg_id in (data[0].split() if data and data[0] else []):
            ...
            # Разрешаем и точную тему, и короткую «Поступление денег».
            ...
            for part in message.walk():
                ...
                if payload:
                    candidates.append((message_at, payload, filename))
        if not candidates:
            raise RuntimeError(
                f"За {LOOKBACK_DAYS} дней не найдено письмо с Excel-вложением и темой «{SUBJECT_MARKER}»"
            )
        message_at, payload, filename = max(candidates, key=lambda x: x[0])
        return payload, filename, message_at
    finally:
        try:
            mail.logout()
        except Exception:
...
def main() -> None:
    ...
    try:
        ...
        if LOCAL_FILE:
            ...
            payload, filename, message_at = latest_attachment()
            log(f"Письмо: {filename}, {message_at.isoformat()}")
