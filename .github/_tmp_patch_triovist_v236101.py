...
# Motivation
p = Path('assets/03-triovist-routes.js')
s = p.read_text(encoding='utf-8')
old = '''  function taskMetrics(manager){
...
new = '''  function taskMetrics(manager){
    ...
    const salaryEligible=all.filter(t=>t.status!=='pending_approval');'''
s = replace_once(s, old, new, 'motivation month task filter')

old = '''    const forecastSales=factor>0&&factor<1?s.current/factor:s.current,forecastPlanPct=plan>0?forecastSales/plan*100:0;
    const growthPct=s.previous>0?(s.current-s.previous)/s.previous*100:(s.current>0?100:0);'''
new = '''    const forecastSales=factor>0&&factor<1?s.current/factor:s.current,forecastPlanPct=plan>0?forecastSales/plan*100:0;
...
s = replace_once(s, old, new, 'motivation normalized growth')
...
s = replace_once(
    ...
    'motivation monthly task rpc'
)
s = replace_once(
    s,
    "<small>${pctFmt(x.growthPct)} к аналогу</small>",
    "<small>${pctFmt(x.growthPct)} · ${monthFactor(selectedMonth())<1?'прогноз к аналогу':'к аналогу'}</small>",
    'motivation growth label'
)
p.write_text(s, encoding='utf-8')

# AI plan
p = Path('assets/11-triovist-ai-plans-v2348.js')
s = p.read_text(encoding='utf-8')
s = replace_once(s, '/* RESANTA CRM v23.4.8 · TRIOVIST AI PLANS · CACHE-SAFE MANAGER ROOT */', '/* RESANTA CRM v23.6.101 · TRIOVIST AI PLANS · sell-out + replenishment gap truth */', 'AI header')
s = replace_once(s, "const V='v23.4.8';", "const V='v23.6.101';", 'AI version')

start = s.index('function prices(m){')
end = s.index('function stockDates(){', start)
block = r'''function prices(m){
  const z=new Map();
...
start = s.index('function recommendation(m){')
end = s.index('\n\nfunction css(){', start)
new_rec = r'''function recommendation(m){
...
start = s.index('function gapHtml(m,gap){')
end = s.index('function leaderHtml(m,rec){', start)
new_gap = r'''function bucketGroups(pack,type){
  let s='';
  (pack.out||[]).forEach(g=>{
...
'''
s = s[:start] + new_gap + s[end:]
s = replace_once(s, 'Товарный резерв ${money(rec.stock)}', 'Коммерческий резерв ${money(rec.stock)}', 'AI reserve label')
p.write_text(s, encoding='utf-8')

# cache/version
p = Path('assets/66-performance-root-v23657.js')
s = p.read_text(encoding='utf-8')
s = replace_once(s, "const V='23.6.100',flights", "const V='23.6.101',flights", 'performance root version')
p.write_text(s, encoding='utf-8')

p = Path('index.html')
s = p.read_text(encoding='utf-8')
s = replace_once(s, "const APP_VERSION = '2026-09-10-resanta-crm-v23.6.100';", "const APP_VERSION = '2026-09-10-resanta-crm-v23.6.101';", 'APP_VERSION')
if s.count('./assets/03-triovist-routes.js?v=23.6.84') != 2:
    raise SystemExit('03 cache query: expected 2 old references')
s = s.replace('./assets/03-triovist-routes.js?v=23.6.84','./assets/03-triovist-routes.js?v=23.6.101')
s = replace_once(s, './assets/66-performance-root-v23657.js?v=23.6.100', './assets/66-performance-root-v23657.js?v=23.6.101', 'performance root cache query')
p.write_text(s, encoding='utf-8')

checks = {
    'assets/03-triovist-routes.js': ["triovist_tasks_get_dashboard_month_v23648", "taskMonth(t)===selected", "growthBase=factor>0&&factor<1?forecastSales:s.current"],
    'assets/11-triovist-ai-plans-v2348.js': ["selloutOpportunities", "if(partnerNet(r)>0)return", "Вернуть продажи на товаре, который уже есть у 21vek", "Пока не покрыто подтверждёнными источниками"],
    'assets/66-performance-root-v23657.js': ["const V='23.6.101'"],
    'index.html': ["resanta-crm-v23.6.101", "03-triovist-routes.js?v=23.6.101"]
}
...
print('v23.6.101 semantic patch checks passed')
