...
def main():
    ...
    for msg_id in ids:
        ...
        try:
            ...
            parsed_candidates.append((month, sent, subject, filename, rows))
        except Exception as exc:
    ...
    month, sent, subject, filename, rows = parsed_candidates[-1]
    ...
    log(f"  Период: {month[:7]}, вложение: {filename}, строк: {len(rows)}")
