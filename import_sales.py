...
def refresh_merged_client_turnover():
    """Пересчитывает оборот объединённых ТТ по всей purchase_history.

    Карточка-дубль после объединения удаляется, но продажи в 1С продолжают
    приходить под её старым названием. Эти названия сохранены как CRM_ALIASES.
    Раньше импорт учитывал алиасы только при защите от повторного создания
    дубля, но не обновлял revenue_total главной карточки. Поэтому 33K + 13K
    визуально оставалось 33K. Теперь после каждого импорта сумма всех алиасов
    записывается в главного клиента.
    """
    headers = {
        "apikey": SUPABASE_KEY,
        "Authorization": f"Bearer {SUPABASE_KEY}",
    ...
    merged = [c for c in clients if len(client_name_variants(c)) > 1]
    if not merged:
        log("  Объединённых ТТ для пересчёта оборота нет.")
        return

    history = _get_all_rest_rows("purchase_history", "client_name,revenue")
    by_name = {}
    for row in history:
        key = normalize_name(row.get("client_name"))
        if not key:
            continue
        by_name[key] = by_name.get(key, 0.0) + float(row.get("revenue") or 0)

    updates = 0
    for client in merged:
        variant_norms = [normalize_name(n) for n in client_name_variants(client)]
        variant_norms = list(dict.fromkeys(v for v in variant_norms if v))
        matched_names = set()

        # Сначала точные нормализованные совпадения — самый безопасный вариант.
        for variant in variant_norms:
            if variant in by_name:
                matched_names.add(variant)

        # Затем допускаем нечёткое совпадение, но только когда оно однозначно
        # относится к одному из алиасов этой карточки.
        for hist_name in by_name:
            if hist_name in matched_names:
                continue
            hits = [v for v in variant_norms if names_match(hist_name, v)]
            if len(hits) == 1:
                matched_names.add(hist_name)

        actual = sum(by_name[name] for name in matched_names)
        ...
        updates += 1
        log(f"  Σ {client.get('name')}: {old:,.2f} -> {actual:,.2f} BYN "
            f"({len(matched_names)} названий ТТ)")

    log(f"  Пересчитан оборот объединённых клиентов: {updates}.")
