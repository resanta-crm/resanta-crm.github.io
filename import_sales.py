...
Версия контроля разбора: v22.7.18 — быстрый IMAP-поиск + сверка каждого контрагента с итогом 1С.
...
def fetch_message_header(mail, msg_id):
    """Загружает только Subject/Date, не всё письмо с тяжёлым Excel-вложением."""
    status, data = mail.fetch(msg_id, "(BODY.PEEK[HEADER.FIELDS (SUBJECT DATE)])")
    if status != "OK":
        return None
    raw = b""
    for item in data or []:
    ...
    return {"id": msg_id, "subject": subject, "sent": normalize_message_date(msg.get("Date"))}


def sales_mail_candidates(mail, ids):
    """Сначала смотрит лёгкие заголовки всех писем, затем возвращает только
    несколько самых свежих писем с нужной темой. Полные XLSX не скачиваются
    для десятков нерелевантных писем.
    """
    out = []
    for msg_id in ids:
        try:
            meta = fetch_message_header(mail, msg_id)
        except Exception as exc:
            log(f"  ⚠️ Заголовок письма {msg_id!r} пропущен: {exc}")
            continue
        if not meta or SUBJECT_MARKER.lower() not in meta["subject"].lower():
            continue
        out.append(meta)
    out.sort(key=lambda x: x["sent"], reverse=True)
    if len(out) > MAX_FULL_MESSAGES:
        log(f"Подходящих писем: {len(out)}. Полностью проверяю только {MAX_FULL_MESSAGES} самых свежих.")
    else:
        log(f"Подходящих писем: {len(out)}. Проверяю их вложения.")
...
def main():
    ...
    ids = data[0].split()
    log(f"Писем за последние {LOOKBACK_DAYS} дн.: {len(ids)}. Сначала проверяю только Subject/Date.")
    candidates = sales_mail_candidates(mail, ids)

    # Тяжёлое письмо с Excel скачиваем только для нескольких самых свежих кандидатов.
    # Из них выбираем максимальный период отчёта, затем максимальную дату окончания
    # и только потом время письма. Это сохраняет защиту на стыке месяцев.
    parsed_candidates = []
    parse_errors = []
    for meta in candidates:
        ...
        try:
            ...
            parsed_candidates.append((month, period_end, sent, subject, filename, rows))
        except Exception as exc:
    ...
    parsed_candidates.sort(key=lambda x: (x[0], x[1], x[2]))
    month, report_end, sent, subject, filename, rows = parsed_candidates[-1]
