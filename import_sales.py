...
Версия контроля разбора: v22.7.17 — сверка каждого контрагента с итогом 1С.
...
def main():
    ...
    # Разбираем все подходящие вложения и выбираем самый новый ПЕРИОД отчёта,
    # а внутри периода — самое свежее письмо. Поэтому закрытие прошлого месяца
    # больше не вытесняет текущий июльский отчёт только из-за времени отправки.
    parsed_candidates = []
    parse_errors = []
    for msg_id in ids:
    ...
    snapshot_ok = save_vip_yoy_snapshot(report_end, month, rows, source_message_at=sent.isoformat())
    if snapshot_ok:
        backfill_previous_year_snapshot(mail, report_end)
