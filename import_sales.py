...
def ensure_clients_exist(rows):
    ...
    existing = resp.json()
    # Включаем прежние названия объединённых торговых точек. Иначе после удаления
    # дубля следующий импорт снова создавал его как нового клиента.
    existing_norm = []
    for c in existing:
        existing_norm.extend(normalize_name(n) for n in client_name_variants(c) if normalize_name(n))
...
def _compact_name(name):
    """Нормализованное имя без пробелов — ловит варианты «Адильханова Трэйд» / «АдильхановаТрэйд»."""
    return normalize_name(name).replace(" ", "")


def refresh_merged_client_turnover():
    """Пересчитывает оборот ВСЕХ клиентов по полной purchase_history.

    Почему не только объединённых:
      старые дубли могли быть объединены до появления CRM_ALIASES. Тогда
      удалённая карточка уже исчезла, а у главной карточки нет списка прежних
      названий. Но продажи в purchase_history остались под обеими юрформами
      (например ЧТУП и ИП). Их безопасно объединять по названию без юрформы.

    Для обычных клиентов используем только точное совпадение нормализованного
    названия (и вариант без пробелов). Нечёткое совпадение разрешаем лишь для
    явно сохранённых алиасов — чтобы не склеить похожие, но разные компании.
    """
    headers = {
        "apikey": SUPABASE_KEY,
        "Authorization": f"Bearer {SUPABASE_KEY}",
    ...
    clients = _get_all_rest_rows("clients", "id,name,assortment,revenue_total")
    if not clients:
        log("  Клиентов для пересчёта оборота нет.")
        return

    history = _get_all_rest_rows("purchase_history", "client_name,revenue")

    # Суммы по точному нормализованному имени и по имени без пробелов.
    by_norm = {}
    by_compact = {}
    compact_to_norms = {}
    for row in history:
        raw_name = row.get("client_name")
        norm = normalize_name(raw_name)
        compact = _compact_name(raw_name)
        if not norm:
            continue
        revenue = float(row.get("revenue") or 0)
        by_norm[norm] = by_norm.get(norm, 0.0) + revenue
        by_compact[compact] = by_compact.get(compact, 0.0) + revenue
        compact_to_norms.setdefault(compact, set()).add(norm)

    updates = 0
    recovered = 0
    for client in clients:
        variants = client_name_variants(client)
        primary_norm = normalize_name(client.get("name"))
        primary_compact = _compact_name(client.get("name"))
        stored_aliases = variants[1:]

        matched_norms = set()

        # 1) Точные совпадения по главному названию и сохранённым алиасам.
        for variant in variants:
            norm = normalize_name(variant)
            compact = _compact_name(variant)
            if norm in by_norm:
                matched_norms.add(norm)
            # Склеиваем вариант с/без пробела только если этот compact однозначен.
            norms = compact_to_norms.get(compact, set())
            if compact and len(norms) == 1:
                matched_norms.update(norms)
            elif compact == primary_compact and norms:
                # Для одной компании разные юрформы после удаления стоп-слов часто
                # дают несколько norm, но одинаковый compact. Это и есть старое
                # объединение, которое надо восстановить.
                matched_norms.update(norms)

        # 2) Нечёткое совпадение — только по явно сохранённым алиасам.
        if stored_aliases:
            alias_norms = [normalize_name(a) for a in stored_aliases if normalize_name(a)]
            for hist_norm in by_norm:
                if hist_norm in matched_norms:
                    continue
                hits = [a for a in alias_norms if names_match(hist_norm, a)]
                if len(hits) == 1:
                    matched_norms.add(hist_norm)

        actual = sum(by_norm[n] for n in matched_norms)
        old = float(client.get("revenue_total") or 0)
        if abs(actual - old) <= 0.01:
            continue

        resp = requests.patch(
            f"{SUPABASE_URL}/rest/v1/clients?id=eq.{client['id']}",
            headers=headers,
            json={"revenue_total": actual},
            timeout=60,
        )
        if resp.status_code not in (200, 204):
            raise RuntimeError(
                f"Не удалось обновить оборот клиента {client.get('name')}: "
                f"{resp.status_code} {resp.text}"
            )
        updates += 1
        if len(matched_norms) > 1 or (not stored_aliases and actual > old):
            recovered += 1
        log(
            f"  Σ {client.get('name')}: {old:,.2f} -> {actual:,.2f} BYN "
            f"({len(matched_norms)} названий из 1С)"
        )

    log(f"  Пересчитан оборот клиентов: {updates}; восстановлено старых объединений: {recovered}.")
...
def main():
    ...
    # После каждой загрузки продаж пересчитываем главные карточки объединённых
    # торговых точек по всем сохранённым названиям. Ошибка здесь должна быть
    # видна в Action: иначе импорт будет зелёным, а оборот в CRM останется старым.
    refresh_merged_client_turnover()
