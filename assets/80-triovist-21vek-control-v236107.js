/* RESANTA CRM v23.6.107 · TRIOVIST OWN 21VEK CONTROL CENTER
...
const VERSION='v23.6.107',TTL=30000;
...
function render(d){
  ...
  root.innerHTML=`
    ...
    ${can?'<div class="tri21ctl-note">Ручной запрос подхватывается сервером максимум примерно за 10 минут. Рабочий снимок меняется только после полного успешного сбора карточек и TOP; при ошибке остаётся последний хороший снимок.</div>':''}`;
}
async function load(force=false){
  const root=ensurePanel();if(!root)return;
  try{render(await status(force));}
  catch(e){console.error('21vek control',e);root.innerHTML='<div class="tri21ctl-error">Не удалось получить статус собственного парсера 21vek: '+esc(e?.message||e)+'</div>';}
}
