/* RESANTA CRM v23.6.27 · ROUTE VISIT EVIDENCE
...
(function(){
'use strict';
if(window.RESANTA_ROUTE_VISIT_EVIDENCE_V23627)return;
const VERSION='v23.6.27',EVIDENCE='verified_visit_evidence',BOSS_OK='boss_confirmed_gps_review',BOSS_NO='boss_rejected_gps_review';
...
function wrapVerified(){
  ...
  if(typeof base!=='function'||base.__routeEvidenceV23627)return false;
  const w=function(r){if(evidenceOk(r))return true;return base.apply(this,arguments)};
  w.__routeEvidenceV23627=true;w.__base=base;window.routePlanVerified=w;try{routePlanVerified=w}catch(_){}return true;
}
function wrapSaver(name){
  ...
  if(typeof base!=='function'||base.__routeEvidenceV23627||!base.__gpsTruthV2360)return false;
  ...
  w.__routeEvidenceV23627=true;w.__gpsTruthV2360=true;w.__base=base;window[name]=w;try{if(name==='saveVisit')saveVisit=w;else saveQuickVisit=w}catch(_){}return true;
}
function cleanFalseReview(){
  const card=document.getElementById('gps-visit-review-v2354');if(!card)return;
  const accepted=routes().filter(evidenceOk);
...
function wrapRender(){
  ...
  if(typeof base!=='function'||base.__routeEvidenceV23627)return false;
  const w=function(){const out=base.apply(this,arguments);setTimeout(cleanFalseReview,0);return out};w.__routeEvidenceV23627=true;w.__base=base;window.renderVisits=w;try{renderVisits=w}catch(_){}return true;
}
function install(){
  wrapVerified();wrapSaver('saveVisit');wrapSaver('saveQuickVisit');wrapRender();
  setTimeout(reconcileRecent,100);
}
[300,900,1900,3500].forEach(ms=>setTimeout(install,ms));
window.RESANTA_ROUTE_VISIT_EVIDENCE_V23627=Object.freeze({version:VERSION,checkinTruth:true,sustainedStopTruth:true,reportGpsKeptForAudit:true,noRadiusInflation:true});
})();
