...
function css(){
 ...
 s.textContent='#wc-inventory-v236139 .iv-head{display:flex;justify-content:space-between;gap:10px;align-items:flex-start;flex-wrap:wrap;margin-bottom:12px}#wc-inventory-v236139 .iv-kpis{display:grid;grid-template-columns:repeat(5,minmax(140px,1fr));gap:9px;margin-bottom:12px}#wc-inventory-v236139 .iv-kpi{border:1px solid var(--border);background:#fff;border-radius:10px;padding:11px}#wc-inventory-v236139 .iv-kpi small{display:block;font-size:10px;color:var(--sub);font-weight:700;text-transform:uppercase}#wc-inventory-v236139 .iv-kpi b{display:block;font-size:19px;margin-top:4px}.iv-tools{display:flex;gap:7px;align-items:center;flex-wrap:wrap;margin-bottom:10px}.iv-tools input,.iv-tools select{padding:9px 10px;border:1px solid var(--border);border-radius:8px;background:#fff}.iv-tools input{flex:1;min-width:190px}.iv-btn{padding:9px 12px;border:1px solid var(--border);border-radius:8px;background:#fff;cursor:pointer;font-weight:700}.iv-btn.primary{background:var(--a);border-color:var(--a);color:#fff}.iv-btn.danger{color:#991B1B;border-color:#FCA5A5}.iv-filter.active{background:var(--ab);color:var(--at);border-color:#60A5FA}.iv-grid{display:grid;grid-template-columns:1fr;gap:12px}.iv-card{background:#fff;border:1px solid var(--border);border-radius:11px;padding:13px}.iv-table{overflow:auto;border:1px solid var(--border);border-radius:10px}.iv-table table{width:100%;border-collapse:collapse;table-layout:fixed;min-width:0}.iv-table th{font-size:12px;color:var(--sub);background:#F8FAFC;padding:8px;text-align:left;border-bottom:1px solid var(--border);white-space:nowrap}.iv-table td{font-size:13px;padding:8px;border-bottom:1px solid var(--border)}.iv-diff-neg{color:#B91C1C;font-weight:800}.iv-diff-pos{color:#166534;font-weight:800}.iv-ok{color:#166534}.iv-muted{font-size:11px;color:var(--sub)}.iv-qr{display:block;width:180px;height:180px;max-width:100%;margin:8px auto;background:#fff}.iv-alert{padding:11px 13px;border-radius:9px;font-size:12px;line-height:1.5;margin-bottom:10px}.iv-blue{background:#EFF6FF;border:1px solid #BFDBFE;color:#1E3A8A}.iv-red{background:#FEF2F2;border:1px solid #FECACA;color:#991B1B}.iv-green{background:#ECFDF5;border:1px solid #A7F3D0;color:#166534}@media(max-width:900px){#wc-inventory-v236139 .iv-kpis{grid-template-columns:repeat(2,1fr)}.iv-table{overflow:auto}.iv-table table{min-width:820px;table-layout:auto}}@media(max-width:560px){#wc-inventory-v236139 .iv-kpis{grid-template-columns:1fr}}';
...
async function renderAudit(){
 const box=$('iv-audit');if(!box||!summary?.session?.id)return;
 try{
  const rows=await rpc('warehouse_inventory_recent_events_manager_v1',{p_session_id:summary.session.id,p_limit:30});
  if(!rows?.length){box.innerHTML='<div class="iv-muted">Сканов пока нет.</div>';return}
  box.innerHTML='<div style="overflow:auto"><table style="width:100%;border-collapse:collapse;min-width:760px"><thead><tr><th style="text-align:left;padding:7px">Время</th><th style="text-align:left;padding:7px">Артикул / товар</th><th style="text-align:left;padding:7px">Действие</th><th style="text-align:left;padding:7px">Кто пикал</th><th style="text-align:left;padding:7px">Где</th></tr></thead><tbody>'+
    rows.map(r=>'<tr style="border-top:1px solid var(--border)"><td style="padding:8px">'+esc(stampRu(r.created_at))+'</td><td style="padding:8px"><b>'+esc(r.sku||'')+'</b><div class="iv-muted">'+esc(r.product||'')+'</div></td><td style="padding:8px">'+esc(r.action_label||'')+(Number(r.delta)!==0?' · '+(Number(r.delta)>0?'+':'')+qty(r.delta):'')+'</td><td style="padding:8px"><b>'+esc(r.actor_name||r.actor_email||'—')+'</b><div class="iv-muted">'+esc(r.actor_email||'')+'</div></td><td style="padding:8px"><b>'+esc(r.device_label||'—')+'</b></td></tr>').join('')+
    '</tbody></table></div>';
 }catch(e){box.innerHTML='<div class="iv-muted">Не удалось загрузить историю ТСД: '+esc(e?.message||e)+'</div>'}
}
