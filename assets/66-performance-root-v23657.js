...
const VERSIONED_GUARDS=Object.freeze({RESANTA_TRIOVIST_AI_PLANS_V2348:'v23.6.103',RESANTA_WAREHOUSE_COMPACT_V23637:'v23.6.141',RESANTA_WAREHOUSE_CONTROL_V23620:'v23.6.154',RESANTA_WAREHOUSE_INVENTORY_V236139:'v23.6.153',RESANTA_WAREHOUSE_RECEIVING_V236154:'v23.6.155',RESANTA_MARKDOWN_V23687:'v23.6.161'});
...
const CONTRACT={
  promotions:[
    {path:'assets/25-promotions-boss-substitute-v2363.js',marker:'perf-promo-flow-v23671',guard:'RESANTA_PROMOTIONS_BOSS_SUBSTITUTE_V2363'},
    {path:'assets/29-promotions-decision-ui-v2368.js',marker:'perf-promo-decision-v23671',guard:'RESANTA_PROMOTIONS_DECISION_UI_V2368'},
    {path:'assets/41-promotions-work-filter-v23626.js',marker:'perf-promo-work-v23671',guard:'RESANTA_PROMOTIONS_WORK_FILTER_V23626'},
    {path:'assets/45-promotions-budget-truth-v23630.js',marker:'perf-promo-budget-truth-v23671',guard:'RESANTA_PROMOTIONS_BUDGET_TRUTH_V23630'},
    {path:'assets/46-promotions-budget-dfs-override-v23631.js',marker:'perf-promo-budget-override-v23671',guard:'RESANTA_PROMOTIONS_BUDGET_DFS_OVERRIDE_V23631'},
    {path:'assets/47-promotions-budget-snapshot-v23632.js',marker:'perf-promo-budget-snapshot-v23671',guard:'RESANTA_PROMOTIONS_BUDGET_SNAPSHOT_V23632'},
    {path:'assets/63-promotions-management-v23654.js',marker:'perf-promo-management-v23671',guard:'RESANTA_PROMOTIONS_MANAGEMENT_V23654'},
    {path:'assets/65-promotions-close-v23656.js',marker:'perf-promo-close-v23671',guard:'RESANTA_PROMOTIONS_CLOSE_V23656'},
...
function promoOverlay(show,text){
  ...
  let el=document.getElementById('promo-contract-loading-v23671');
  if(!show){el?.remove();return}
  if(!root.style.position)root.style.position='relative';
  if(!el){
    el=document.createElement('div');
    el.id='promo-contract-loading-v23671';
    ...
    el.innerHTML='<div class="card" style="width:min(620px,calc(100% - 32px));padding:22px;text-align:center"><div style="font-size:20px;font-weight:800;margin-bottom:7px">🎯 Акции</div><div id="promo-contract-loading-text-v23671" style="font-size:13px;color:var(--sub)">Загружаю актуальные акции…</div></div>';
    root.appendChild(el);
  }
  const t=document.getElementById('promo-contract-loading-text-v23671');if(t&&text)t.textContent=text;
}
...
async function loadPromotions(){
  return onceContract('promotions',async()=>{
    ...
    Promise.resolve(window.crmPromotionSubstitutionsReadyV23671).catch(()=>{});
...
window.crmPromotionsReadyV23671=()=>promotionsReady&&promotionsGuardsReady();
...
async function loadTriovist(){
  await Promise.all([
    load('assets/11-triovist-ai-plans-v2348.js','perf-tri-ai-v23671','RESANTA_TRIOVIST_AI_PLANS_V2348'),
...
async function loadForPage(page,epoch){
  ...
  try{
    ...
    if(p==='payments'||p==='debt')return await load('assets/12-finance-data-root-v2349.js','perf-finance-v23671','RESANTA_FINANCE_DATA_ROOT_V2349');
    if(p==='managers'){
      await load('assets/07-ai-manager-plans.js','perf-manager-ai-v23671','RESANTA_AI_MANAGER_PLANS_V2320');
      await load('assets/08-manager-gap-sources.js','perf-manager-gap-v23671','RESANTA_MANAGER_GAP_SOURCES_V2322');
...
window.crmModuleContractCheckV23671=function(){
  return {
    version:'v'+V,
...
window.RESANTA_PERFORMANCE_ROOT_V23657=Object.freeze({
  ...
  promotionsStack:['25','29','41','45','46','47','63','65','72'],
  promotionsDataBeforeFinalRender:true,promotionsSingleFinalRender:true,promotionsBoundedModuleWait:true,promotionsDuplicatePaintGuard:true,
  warehouseShell:'36',
