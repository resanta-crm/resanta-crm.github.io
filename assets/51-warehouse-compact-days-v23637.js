/* RESANTA CRM v23.6.38 · WAREHOUSE COMPACT TABLE + FIXED 14-DAY HORIZON
 * UI-only presentation for warehouse rows.
 * Server is the source of truth for calculations; order horizon is locked to 14 days in DB.
 * No polling, no MutationObserver, no calculation overrides.
 */
(function(){
'use strict';
...
#wc-v23620 .wc-compact .c-sku{width:39%}.wc-compact .c-vb{width:7%}.wc-compact .c-days{width:9%}.wc-compact .c-need{width:9%}.wc-compact .c-ch{width:8%}.wc-compact .c-box{width:6%}.wc-compact .c-order{width:9%}.wc-compact .c-cost{width:13%}
@media(max-width:1050px){#wc-v23620 .wc-compact{min-width:880px!important}#wc-v23620 .wc-compact .c-sku{width:330px}}
...
function daysCell(r,data){
 const target=horizonOf(r,data);
 const d=r.days_cover;
 if(d===null||d===undefined||!Number.isFinite(Number(d)))return `<span class="wc-days">—</span><div class="wc-mini">норма ${target} дн.</div>`;
 const x=Math.max(0,Number(d));const cls=x<=3?'bad':x<=7?'warn':'good';
 return `<span class="wc-days ${cls}">${x.toLocaleString('ru-RU',{maximumFractionDigits:1})} дн.</span><div class="wc-mini">норма ${target} дн.</div>`
}
...
async function load(){
 const root=$('wc-body');if(!root||mode==='overview')return;
...
function render(data){
 const root=$('wc-body');if(!root)return;
 ...
 if(mode==='order'||mode==='stockout'){
  ...
  body=rows.map(r=>`<tr><td class="wc-sku"><b>${esc(r.sku)}</b><div>${esc(r.product||'')}</div></td><td class="${n(r.vitebsk_avail)<=0?'wc-bad':''}">${qty(r.vitebsk_avail??r.closing_qty)}</td><td>${daysCell(r,data)}</td><td class="wc-warn"><b>${qty(r.need_qty)}</b><div class="wc-mini">прогноз ${qty(r.forecast_qty)} · цель ${qty(r.target_qty)}</div></td><td>${qty(r.chekhov_qty)}</td><td>${qty(r.box_qty)}</td><td class="wc-good">${qty(r.recommended_order_qty)}</td><td>${money(r.estimated_order_cost_byn)}<div class="wc-mini">${money(r.estimated_order_cost_rub,'RUB')}</div></td></tr>`).join('')
 }else if(mode==='excess'){
  head='<th class="c-sku">SKU / товар</th><th>Витебск</th><th>Дней</th><th>Излишек</th><th>Излишек BYN</th><th>Решение</th>';
  body=rows.map(r=>{const ret=r.return_allowed?'<span class="wc-warn">Остановить заказ / рассмотреть возврат</span>':'<span class="wc-bad">Группа 900 — возврат запрещён</span>';return `<tr><td class="wc-sku"><b>${esc(r.sku)}</b><div>${esc(r.product||'')}</div></td><td>${qty(r.vitebsk_avail??r.closing_qty)}</td><td>${daysCell(r,data)}<div class="wc-mini">прогноз ${qty(r.forecast_qty)} · норма ${qty(r.norm_target_qty)}</div></td><td class="wc-bad">${qty(r.excess_qty)}</td><td class="wc-bad">${money(r.excess_cost_byn)}<div class="wc-mini">${money(r.excess_cost_rub,'RUB')}</div></td><td>${ret}</td></tr>`}).join('')
 }else{
  ...
  body=rows.map(r=>`<tr><td class="wc-sku"><b>${esc(r.sku)}</b><div>${esc(r.product||'')}</div></td><td>${qty(r.vitebsk_avail??r.closing_qty)}</td><td>${daysCell(r,data)}</td><td>${qty(r.expense_qty)}</td><td>${qty(r.forecast_qty)}</td><td>${qty(r.target_qty)}</td><td>${qty(r.need_qty)}</td><td>${qty(r.excess_qty)}</td><td>${qty(r.chekhov_qty)}</td></tr>`).join('')
 }
 if(!body)body='<tr><td colspan="9" style="text-align:center;color:var(--sub);padding:20px">По выбранному фильтру товаров нет.</td></tr>';
 root.innerHTML=`<div class="wc-tools"><b style="font-size:14px">${esc(title())}</b>${(mode==='order'||mode==='stockout')?`<span class="wc-horizon">Цель запаса: ${horizon} дней</span>`:''}<input id="wc-search" placeholder="Артикул или товар" value="${esc(search)}"><button id="wc-find">Найти</button>${mode==='order'?'<button class="primary" id="wc-export">⬇ Скачать автозаказ Чехов</button>':''}</div><div class="wc-table-wrap"><table class="wc-compact"><thead><tr>${head}</tr></thead><tbody>${body}</tbody></table></div><div class="wc-page"><span class="wc-muted">Показано ${rows.length} из ${total}</span><div><button id="wc-prev" ${offset<=0?'disabled':''}>← Назад</button> <button id="wc-next" ${offset+limit>=total?'disabled':''}>Вперёд →</button></div></div>`;
 const find=$('wc-find'),inp=$('wc-search'),prev=$('wc-prev'),next=$('wc-next');
 if(find)find.onclick=()=>{search=inp?.value.trim()||'';offset=0;load()};if(inp)inp.onkeydown=e=>{if(e.key==='Enter')find?.click()};if(prev)prev.onclick=()=>{offset=Math.max(0,offset-limit);load()};if(next)next.onclick=()=>{offset+=limit;load()};if($('wc-export'))$('wc-export').onclick=exportOrder
}
...
window.RESANTA_WAREHOUSE_COMPACT_V23637=Object.freeze({version:V,daysCover:true,fixedOrderHorizonDays:FIXED_HORIZON,stickyHeader:true,stickySku:true,noPolling:true,noMutationObserver:true,calculationChanged:false});
})();
