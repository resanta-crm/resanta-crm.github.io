...
function allowedManager(k){
  const map={home:['triovist-warning','tri-kpis','tri-manager-cards','tri-plans-card'],sales:['triovist-warning','tri-period-mode','tri-kpis','tri-manager-cards','tri-groups','tri-alerts','tri-details'],stock:['tri-stock-card'],tasks:['tri-v22728-pane-tasks','tri-assignments-card'],motivation:['tri-ai-independent-root-v2348','tri-v22728-pane-motivation','tri-motivation-card','tri-plans-card'],cards:['tri-v22728-pane-cards']};
  const set=new Set();for(const id of map[k]||[]){const n=idNode(id);if(n)set.add(n);}if(k==='sales'){const p=findPeriodTop();if(p)set.add(p);}return set;
}
function showManagerWork(k){restoreHidden();setActive(k);const allow=allowedManager(k),known=managerKnownNodes();for(const n of known)if(!allow.has(n))hideNode(n);window.scrollTo({top:0,behavior:'auto'});}
function showLeaderWork(k){restoreHidden();setActive(k);const b=legacyButton(k);if(b){try{b.click();}catch(_){}}window.scrollTo({top:0,behavior:'auto'});}
function showWork(k){if(ctx?.is_manager&&!legacyHost)showManagerWork(k);else showLeaderWork(k);}

function concealForCommercial(){restoreHidden();ensurePanel();const children=[...page.children],idx=children.indexOf(shell);for(let i=idx+1;i<children.length;i++){const el=children[i];if(el===panel)continue;hideNode(el);}panel.style.display='block';}
...
async function renderExpense(kind,m){const d=await getMonth(m||ym()),rows=(d.expenses||[]).filter(x=>x.expense_type===kind),total=rows.reduce((s,x)=>s+Number(x.amount||0),0),write=!!d.can_write_expenses;panel.innerHTML='<div class="tri-v23612-section"><div><h3>'+(kind==='anp'?'🧾 АНП':'🛠 Компенсация СИ')+'</h3><div class="tri-v23612-note">Фактические затраты по месяцам. '+(write?'Можно вносить расходы и прикладывать документы.':'Режим просмотра.')+'</div></div>'+monthPicker(kind,d.month)+'</div><div class="tri-v23612-kpis"><div class="tri-v23612-kpi"><span>'+(kind==='anp'?'АНП':'СИ')+' за месяц</span><b>'+money(total)+'</b></div><div class="tri-v23612-kpi"><span>АНП + СИ факт</span><b>'+money(d.expenses_total)+'</b></div><div class="tri-v23612-kpi"><span>Документов</span><b>'+Number(d.documents_count||0)+'</b></div><div class="tri-v23612-kpi"><span>Доступ</span><b style="font-size:15px">'+(write?'Внесение':'Просмотр')+'</b></div></div>'+(write?'<div class="tri-v23612-form"><div><label class="form-label">Дата</label><input id="tri-v23612-exp-date" class="form-input" type="date" value="'+esc(String(d.month||first()).slice(0,7)+'-01')+'"></div><div><label class="form-label">Сумма, BYN</label><input id="tri-v23612-exp-amount" class="form-input" type="number" min="0" step="0.01" placeholder="0,00"></div><div><label class="form-label">Комментарий / основание</label><input id="tri-v23612-exp-comment" class="form-input" placeholder="Что компенсировали / на что АНП"></div><button type="button" class="btn-primary" data-v23612-save-exp="'+kind+'">Сохранить</button></div>':'<div class="tri-v23612-info">Просмотр без права изменения.</div>')+'<div style="overflow:auto"><table class="tri-v23612-table"><thead><tr><th>Дата</th><th>Сумма</th><th>Комментарий</th><th>Кто внёс</th><th>Документы</th></tr></thead><tbody>'+(rows.length?rows.map(r=>'<tr><td><b>'+esc(r.expense_date)+'</b></td><td><b>'+money(r.amount)+'</b></td><td>'+esc(r.comment||'—')+'</td><td>'+esc(r.created_by||'—')+'</td><td>'+docsHtml(r,write)+'</td></tr>').join(''):'<tr><td colspan="5">За выбранный месяц расходов ещё нет.</td></tr>')+'</tbody></table></div>';}
async function renderBudget(m){const d=await getMonth(m||ym()),gross=Number(d.sales_revenue||0),net=Number(d.sales_revenue_ex_vat||gross/1.2),norm=Math.round(net*0.015*100)/100;panel.innerHTML='<div class="tri-v23612-section"><div><h3>💼 Бюджет Triovist</h3><div class="tri-v23612-note">Норматив считается от продаж без НДС 20%. Менеджеры видят бюджет, меняет сумму только руководитель.</div></div>'+monthPicker('budget',d.month)+'</div><div class="tri-v23612-kpis"><div class="tri-v23612-kpi"><span>Бюджет месяца</span><b>'+money(d.budget_amount)+'</b></div><div class="tri-v23612-kpi"><span>Продажи с НДС</span><b>'+money(gross)+'</b></div><div class="tri-v23612-kpi"><span>Продажи без НДС 20%</span><b>'+money(net)+'</b></div><div class="tri-v23612-kpi"><span>Норматив 1,5%</span><b>'+money(norm)+'</b></div></div><div class="tri-v23612-info"><b>Комментарий руководителя:</b><br>'+esc(d.budget_comment||'Комментарий пока не добавлен.')+'</div>'+(d.is_leader?'<div class="tri-v23612-form" style="grid-template-columns:190px minmax(250px,1fr) auto"><div><label class="form-label">Сумма бюджета, BYN</label><input id="tri-v23612-budget-amount" class="form-input" type="number" min="0" step="0.01" value="'+Number(d.budget_amount||0).toFixed(2)+'"></div><div><label class="form-label">Комментарий</label><input id="tri-v23612-budget-comment" class="form-input" value="'+esc(d.budget_comment||'')+'"></div><button class="btn-primary" type="button" data-v23612-save-budget>Сохранить бюджет</button></div>':'');}
function priceRange(d){const total=Number(d.total_filtered||0),off=Number(d.offset||0),n=(d.items||[]).length;return total?(off+1)+'–'+(off+n)+' из '+total:'0 из 0';}
async function renderPrice(opts={}){priceState={...priceState,...opts};const d=await rpc('triovist_price_search_v23612',{p_query:priceState.q||'',p_only_suspect:!!priceState.only,p_offset:Number(priceState.offset||0),p_limit:Number(priceState.limit||50)});priceState.last=d;priceState.offset=Number(d.offset||0);priceState.limit=Number(d.limit||priceState.limit);const rows=d.items||[],prev=priceState.offset>0,next=priceState.offset+rows.length<Number(d.total_filtered||0);panel.innerHTML='<div class="tri-v23612-section"><div><h3>🧮 Расчёт цены Triovist</h3><div class="tri-v23612-note">Источник: 1С «Мелкий опт 2 с НДС». Скидка Triovist — 5%. Все '+Number(d.total_all||0)+' SKU доступны.</div></div></div><div class="tri-v23612-search"><input id="tri-v23612-price-q" class="form-input" placeholder="Введите артикул или название товара" value="'+esc(priceState.q||'')+'"><button type="button" class="btn-primary" data-v23612-price-search>Найти</button></div><div class="tri-v23612-price-tools"><button type="button" class="tri-v23612-filter '+(priceState.only?'active':'')+'" data-v23612-only>⚠ Только сильные отклонения ('+Number(d.suspect_count||0)+')</button><span class="tri-v23612-note">Показывать:</span><select class="tri-v23612-select" data-v23612-limit><option value="50" '+(priceState.limit===50?'selected':'')+'>50</option><option value="100" '+(priceState.limit===100?'selected':'')+'>100</option><option value="500" '+(priceState.limit>=500?'selected':'')+'>Все</option></select><button class="tri-v23612-pagebtn" data-v23612-prev '+(!prev?'disabled':'')+'>← Назад</button><span class="tri-v23612-note"><b>'+priceRange(d)+'</b></span><button class="tri-v23612-pagebtn" data-v23612-next '+(!next?'disabled':'')+'>Вперёд →</button></div>'+(Number(d.suspect_count||0)?'<div class="tri-v23612-warn">⚠ Во всём прайсе найдено <b>'+Number(d.suspect_count)+'</b> цен 21vek, которые резко отличаются от истории или находятся существенно ниже закупочной цены Triovist. Используй фильтр выше.</div>':'')+'<div class="tri-v23612-note">'+(d.import?'Прайс: '+esc(d.import.source_file||'')+' · '+Number(d.import.row_count||0)+' SKU':'Прайс ещё не импортирован')+'</div><div style="overflow:auto"><table class="tri-v23612-table"><thead><tr><th>Артикул</th><th>Товар</th><th>Мелкий опт 2 с НДС</th><th>Triovist −5%</th><th>Цена 21vek</th></tr></thead><tbody>'+rows.map(r=>'<tr data-v23612-sku="'+esc(r.sku)+'"><td><b>'+esc(r.sku)+'</b></td><td>'+esc(r.product)+'</td><td><b>'+money(r.list_price)+'</b></td><td>'+money(r.triovist_price)+'</td><td class="'+(r.price_21vek_suspect?'tri-v23612-suspect':'')+'">'+(r.price_21vek==null?'—':(r.price_21vek_suspect?'⚠ ':'')+money(r.price_21vek))+(r.price_21vek_suspect&&r.price_21vek_previous!=null?'<div class="tri-v23612-smallbad">ранее '+money(r.price_21vek_previous)+'</div>':'')+'</td></tr>').join('')+'</tbody></table></div><div id="tri-v23612-price-detail"></div>';}
async function renderPriceDetail(sku){const root=document.getElementById('tri-v23612-price-detail');if(!root)return;const d=await rpc('triovist_price_search_v23612',{p_query:sku,p_only_suspect:false,p_offset:0,p_limit:20}),r=(d.items||[]).find(x=>String(x.sku)===String(sku));if(!r)return;let priv='';if(ctx?.is_leader){const p=await rpc('triovist_price_calc_private',{p_sku:sku,p_point_zero:null});const has=Number(p.point_zero||0)>0,diff=Number(p.difference_to_required||0),status=!has?'Точка 0 не сохранена':diff>=0?'🟢 Цена безопасна':'🔴 Цена ниже допустимой';priv='<div class="tri-v23612-private"><b>🔒 Закрытая экономика — только руководитель</b><div class="tri-v23612-private-grid"><div class="tri-v23612-private-cell"><span>Точка 0</span><b>'+(has?money(p.point_zero):'—')+'</b></div><div class="tri-v23612-private-cell"><span>Минимальная отпускная</span><b>'+(p.required_list_price==null?'—':money(p.required_list_price))+'</b></div><div class="tri-v23612-private-cell"><span>Фактическая отпускная</span><b>'+money(p.list_price)+'</b></div><div class="tri-v23612-private-cell"><span>Отклонение</span><b class="'+(has?(diff>=0?'tri-v23612-good':'tri-v23612-bad'):'')+'">'+(!has?'—':(diff>=0?'+':'−')+money(Math.abs(diff)))+'</b></div></div><div style="margin-top:9px;font-weight:800" class="'+(has?(diff>=0?'tri-v23612-good':'tri-v23612-bad'):'')+'">'+status+'</div><div class="tri-v23612-form" style="grid-template-columns:200px auto minmax(240px,1fr)"><div><label class="form-label">Точка 0, BYN</label><input id="tri-v23612-point-zero" class="form-input" type="number" min="0" step="0.01" value="'+(has?Number(p.point_zero).toFixed(2):'')+'"></div><button type="button" class="btn-secondary" data-v23612-save-zero="'+esc(sku)+'">💾 Сохранить точку 0</button><div class="tri-v23612-note">Обязательства: 9,5%; скидка Triovist: 5%.</div></div></div>'; }root.innerHTML='<div style="border:1px solid #E5E7EB;border-radius:12px;padding:13px;margin-top:12px"><div style="font-size:16px;font-weight:800">'+esc(r.sku)+' · '+esc(r.product)+'</div><div class="tri-v23612-kpis"><div class="tri-v23612-kpi"><span>Мелкий опт 2 с НДС</span><b>'+money(r.list_price)+'</b></div><div class="tri-v23612-kpi"><span>Скидка Triovist</span><b>5%</b></div><div class="tri-v23612-kpi"><span>Цена Triovist</span><b>'+money(r.triovist_price)+'</b></div><div class="tri-v23612-kpi"><span>21vek</span><b>'+(r.price_21vek==null?'—':money(r.price_21vek))+'</b></div></div></div>'+priv;root.scrollIntoView({block:'nearest',behavior:'smooth'});}
async function showCommercial(k){concealForCommercial();setActive(k);panel.innerHTML='<div class="tri-v23612-note">Загрузка…</div>';try{if(k==='anp'||k==='si')await renderExpense(k,ym());else if(k==='budget')await renderBudget(ym());else{priceState={q:'',only:false,offset:0,limit:50,last:null};await renderPrice();}}catch(e){panel.innerHTML='<div class="tri-v23612-warn"><b>Не удалось открыть раздел.</b><br>'+esc(e?.message||e)+'</div>';}}
...
function bindPanel(){if(!panel||panel.dataset.bound23612)return;panel.dataset.bound23612='1';panel.addEventListener('change',async e=>{try{const m=e.target.closest('[data-v23612-month]');if(m){if(m.dataset.v23612Month==='budget')await renderBudget(m.value);else await renderExpense(m.dataset.v23612Month,m.value);return;}const a=e.target.closest('[data-v23612-attach]');if(a){await attachDoc(a.dataset.v23612Attach,a);a.value='';return;}const lim=e.target.closest('[data-v23612-limit]');if(lim){priceState.limit=Number(lim.value);priceState.offset=0;await renderPrice();return;}}catch(err){alert(err.message||err);}});panel.addEventListener('click',async e=>{try{const s=e.target.closest('[data-v23612-save-exp]');if(s){await saveExpense(s.dataset.v23612SaveExp);return;}const d=e.target.closest('[data-v23612-doc]');if(d){await openDoc(d.dataset.v23612Doc);return;}if(e.target.closest('[data-v23612-save-budget]')){await saveBudget();return;}if(e.target.closest('[data-v23612-price-search]')){priceState.q=document.getElementById('tri-v23612-price-q')?.value||'';priceState.offset=0;await renderPrice();return;}if(e.target.closest('[data-v23612-only]')){priceState.only=!priceState.only;priceState.offset=0;await renderPrice();return;}if(e.target.closest('[data-v23612-prev]')){priceState.offset=Math.max(0,priceState.offset-priceState.limit);await renderPrice();return;}if(e.target.closest('[data-v23612-next]')){priceState.offset=priceState.offset+priceState.limit;await renderPrice();return;}const r=e.target.closest('[data-v23612-sku]');if(r){await renderPriceDetail(r.dataset.v23612Sku);return;}const z=e.target.closest('[data-v23612-save-zero]');if(z){await saveZero(z.dataset.v23612SaveZero);return;}}catch(err){alert(err.message||err);}});panel.addEventListener('keydown',e=>{if(e.key==='Enter'&&e.target.id==='tri-v23612-price-q'){e.preventDefault();priceState.q=e.target.value||'';priceState.offset=0;renderPrice().catch(err=>alert(err.message||err));}});}
async function ensure(){if(busy)return false;busy=true;try{injectCss();if(!ctx){ctx=await loadContext();if(!ctx)return false;}if(!locate())return false;ensureShell();ensurePanel();hideLegacy();return true;}finally{busy=false;}}
async function refreshAfterMutation(){const oldShell=shell;if(!await ensure())return;if(!oldShell?.isConnected){shell=null;panel=null;ensureShell();ensurePanel();}hideLegacy();if(ctx?.is_manager&&!legacyHost&&['home','sales','stock','tasks','motivation','cards'].includes(active))showManagerWork(active);}
async function start(){if(!await ensure())return false;setTimeout(()=>activate(saved(),false),120);if(!observer){let t=null;observer=new MutationObserver(()=>{clearTimeout(t);t=setTimeout(refreshAfterMutation,100);});observer.observe(document.documentElement,{childList:true,subtree:true});}window.RESANTA_TRIOVIST_SINGLE_V23612=Object.freeze({version:V,rootManagerAnchorFix:true,serverIdentityWithFallback:true,managerSectionsById:true,fullPricePaging:true,priceAllMode:true,anomalyFilter:true,budgetExVat:true});console.info('RESANTA Triovist '+V+' installed',ctx);return true;}
