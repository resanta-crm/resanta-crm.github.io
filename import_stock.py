...
def load_previous_stock():
    """Read current Vitebsk stock before replacement. Used only to detect genuinely new SKUs."""
    r=requests.get(
        f"{SUPABASE_URL}/rest/v1/stock_balances?warehouse=eq.{WAREHOUSE}&select=sku,product,qty_avail,report_date",
        headers=_rest_headers(), timeout=120,
    )
    if r.status_code!=200:
        log(f"⚠️ Не удалось прочитать предыдущий остаток для новинок: {r.status_code}")
        return {}
    return {str(x.get("sku") or "").strip():x for x in (r.json() or []) if x.get("sku")}


def save_novelties(rows, previous):
    """A novelty is a SKU absent from the previous Vitebsk snapshot and now available > 0.
    Registry has PK sku, so a replenishment can never become a novelty twice. Group 900 is ignored.
    """
    ...
    for x in rows:
        ...
        if avail<=0 or sku in previous:
            continue
        out.append({
            "sku":sku,"product":x.get("product"),"first_seen_date":x.get("report_date") or today,
            "first_seen_qty":avail,"source":"stock_balances","is_legacy":False,
    ...
    for i in range(0,len(out),500):
        r=requests.post(
            f"{SUPABASE_URL}/rest/v1/triovist_novelties?on_conflict=sku",headers=headers,json=out[i:i+500],timeout=120
        )
        if r.status_code not in (200,201,204):
            log(f"⚠️ Новинки не сохранены: {r.status_code} {r.text[:300]}")
            return
    log(f"🆕 Новых SKU Витебска: {len(out)}")
...
def main():
    ...
    replace_stock(rows)
    save_novelties(rows, previous_stock)
