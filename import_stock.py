...
v22.7.30.4.1 — восстановление автоматического обновления Витебска:
...
- логика реестра новинок v22.7.28 сохранена без изменений (её отдельно дорабатываем в задаче №5).
"""
...
def save_novelties(rows, previous):
    """v22.7.28 behaviour preserved intentionally; task №5 will refine it."""
    ...
    for x in rows:
        ...
        out.append({
            ...
            "first_seen_date": x.get("report_date") or today,
            "first_seen_qty": avail,
    ...
    for i in range(0, len(out), 500):
        r = requests.post(
            f"{SUPABASE_URL}/rest/v1/triovist_novelties?on_conflict=sku",
            headers=headers, json=out[i:i + 500], timeout=120,
        )
        if r.status_code not in (200, 201, 204):
...
def main():
    ...
    try:
        ...
        save_novelties(rows, previous_map)
        log(
