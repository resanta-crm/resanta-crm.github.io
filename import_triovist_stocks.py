...
GROUP900_RE = re.compile(r"^900(?:/|$)", re.I)


def is_group900_sku(value):
    """True only for the discontinued internal 900 group (900/...).

    Product model names containing 900 (for example P-900K or TEP-9000)
    are NOT group 900 and must remain in the report.
    """
    return bool(GROUP900_RE.match(str(value or "").strip().lstrip("'")))
...
def parse_partner(path):
    ...
    c_transit = col_any("В пути", "Свободно в пути", "Свободно в пути и резерве", "В пути и резерве")
    try:
        c_reserve = col_any("Резерв")
    except RuntimeError:
        c_reserve = None
    ...
    for idx, values in enumerate(ws.iter_rows(values_only=True)):
        ...
        if not sku:
            ...
            continue
        # Группа 900 снята с рабочего контура Triovist. Исторические продажи
        # не удаляем, но свежие остатки/рекомендации по этим SKU не загружаем.
        ...
        rows.append({
            ...
            "qty_total": n3(dec(values[c_total]) + dec(values[c_transit])), "qty_free": n3(dec(values[c_free]) + dec(values[c_transit])),
            "qty_transit_reserve": n3(dec(values[c_transit]) + (dec(values[c_reserve]) if c_reserve is not None else Decimal("0"))), "qty_orders": n3(values[c_orders]),
            "excluded": excluded,
...
def parse_chekhov(path):
    ...
    for values in data_rows:
        ...
        if not re.fullmatch(r"\d+(?:/\d+){1,6}", sku):
            continue
        # В файле Чехова группа «Распродажа» имеет артикулы 900/... .
        # Не ищем число 900 в названии товара: модели П-900, ТЭП-9000 и т.п.
        # являются обычным ассортиментом и должны остаться.
        if is_group900_sku(sku):
            excluded_900 += 1
            continue
        if sku in seen:
    ...
    log(f"Чехов: группа 900 исключена до загрузки — {excluded_900} SKU")
    return rows
